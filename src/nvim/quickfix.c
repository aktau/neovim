...
static int qf_init_ext(qf_info_T *qi, int qf_idx, const char *restrict efile, buf_T *buf,
                       typval_T *tv, const char *restrict errorformat, bool newlist, linenr_T lnumfirst,
                       linenr_T lnumlast, const char *restrict qf_title, const char *restrict enc)
  FUNC_ATTR_NONNULL_ARG(1)
{
  qfstate_T state = { 0 };
  qffields_T fields = { 0 };
...
void ex_make(exarg_T *eap)
{
  ...
  int res = 0;
  if (*p_mef == NUL && *p_sp == NUL) {
    // If &shellpipe and &makeef are unset, read from the stdout instead of a
    // temporary file.
    //
    // TODO: Detect whether `grepprg` does uses shell constructs [1] and run
    //       it without a shell if not.
    //
    // TODO: If &makeef is unset but not &shellpipe, perhaps just ignore
    // shellpipe.
    //
    // [1]: An example that does use shell constructs:
